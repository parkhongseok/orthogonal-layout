...
// 두 노드의 상대 위치에 따라 가능한 연결 면(side) 조합을 반환합니다.
function getCandidateSides(a: Node, b: Node): [PortSide, PortSide][] {
  ...
  if (Math.abs(dx) > Math.abs(dy)) {
    // 주로 수평 관계
    return [["right", "left"], ["top", "top"], ["bottom", "bottom"]];
  } else {
    // 주로 수직 관계
    return [["bottom", "top"], ["right", "right"], ["left", "left"]];
  }
}
...
export function findBestPortPair(sourceNode: Node, targetNode: Node) {
  ...
  for (const [sourceSide, targetSide] of candidateSides) {
    const sourcePorts = (sourceNode.ports || []).filter(p => p.side === sourceSide);
    const targetPorts = (targetNode.ports || []).filter(p => p.side === targetSide);
  ...
  if (!bestPair) {
    ...
    return {
        sourcePort: center(sourceNode), targetPort: center(targetNode),
        sourceSide: sSide, targetSide: tSide
    };
  }
