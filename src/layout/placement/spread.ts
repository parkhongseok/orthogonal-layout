 ...
 */
export function spread(g: Graph, cfg: any): Graph {
  ...
  for (const [key, nodes] of within) {
    ...
    // 컨테이너 bounds
    const container: Rect = key === rootKey
    ...
    for (let it = 0; it < iterations; it++) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          if (push.dx !== 0 || push.dy !== 0) {
            // 양쪽으로 나눠 밀기
            const ax = clamp(a.bbox.x - push.dx * step, container.x, container.x + container.w - a.bbox.w);
            const ay = clamp(a.bbox.y - push.dy * step, container.y, container.y + container.h - a.bbox.h);
            const bx = clamp(b.bbox.x + push.dx * step, container.x, container.x + container.w - b.bbox.w);
            const by = clamp(b.bbox.y + push.dy * step, container.y, container.y + container.h - b.bbox.h);

            out.nodes.set(a.id, { ...a, bbox: { ...a.bbox, x: snap(ax, grid), y: snap(ay, grid) } });
            out.nodes.set(b.id, { ...b, bbox: { ...b.bbox, x: snap(bx, grid), y: snap(by, grid) } });
            moved++;
          }
        }
      }
      ...
      for (const n of nodes) {
        ...
        const nx = clamp(cur.bbox.x, container.x, container.x + container.w - cur.bbox.w);
        const ny = clamp(cur.bbox.y, container.y, container.y + container.h - cur.bbox.h);
        if (nx !== cur.bbox.x || ny !== cur.bbox.y) {
          out.nodes.set(cur.id, { ...cur, bbox: { ...cur.bbox, x: nx, y: ny } });
        }
      }
...
function resolvePair(a: Rect, b: Rect, gapX: number, gapY: number) {
  ...
  const ax1 = a.x - gapX / 2, ax2 = a.x + a.w + gapX / 2;
  const ay1 = a.y - gapY / 2, ay2 = a.y + a.h + gapY / 2;

  const bx1 = b.x - gapX / 2, bx2 = b.x + b.w + gapX / 2;
  const by1 = b.y - gapY / 2, by2 = b.y + b.h + gapY / 2;
...
function updateGroupBBox(out: Graph, grid: number, inset: number) {
  for (const [gid, g] of out.groups) {
    const children = g.children.map(id => out.nodes.get(id)).filter(Boolean) as Node[];
